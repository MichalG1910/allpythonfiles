...
class Board:
    ...
    def __init__(self, player):
        ...
        self.player = player
        self.x = None # początkowo ustawione na None
        self.y = None
        self.slant = None
    ...
    def check_if_win(self): # definicja sprawdzenia warunków wygranej
        for x in range(0,3): # iteracja x z listy [0,1,2]
            if self.board[x][0] == self.board[x][1] and self.board[x][1] == self.board[x][2] and (self.board[x][2] == "X" or self.board[x][2] == "O"):
                self.x = x # jeżeli instrukcja if zostanie spelniona, zmienia się wartość logiczna self.x na x
    ...
    def check_if_free(self, x, y): # funkcja sprawdza, czy dane pole jest puste self.board[y-1][x-1] == "."
        return self.board[x-1][y-1] == "." # x, y będą wprowadzane z klawiatury, więc trzeba odjąc od nich 1, aby współrzędne odpowiadały indeksowi 
    ...
    def put_to_board(self, x, y): # dodanie naszego X lub O
        if self.check_if_win(): # JEZELI check_if_win == True to przerwie wykonywanie funkcji put_to_board - nie umieścimy już żadnego znaku na naszej tablicy
            pass
            
        elif self.check_if_free(x, y): # sprawdza, czy pole puste (patrz opis check_if_free)
            self.board[x-1][y-1] = self.player # wprowadzenie X lub O (self.player równy jest "X" lub "O") - zastąpienie elementu listy "." na X lub O
